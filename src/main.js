...
(function (window) {
  ...
  function MusicMeta({ title, artist, album, slug }) {
    this.title = title;
    this.src = `${baseUrl}/assets/${slug}/bensound-${slug}.mp3`;
    this.artist = artist;
    this.album = album;
    this.artwork = [96, 128, 192, 256, 512].map(size => {
      return {
        src: `${baseUrl}/assets/${slug}/${slug}-${size}_x_${size}.jpg`,
        sizes: `${size}x${size}`,
        type: 'image/jpg'
      }
    });
...
(function (window, navigator) {
  ...
  function seekBackward(event) {
    audio.currentTime = Math.max(audio.currentTime - (event.seekOffset || defaultSeekTime), 0);
    updatePositionState();
  }
  navigator.mediaSession.setActionHandler('seekbackward', seekBackward);

  function seekForward(event) {
    audio.currentTime = Math.min(audio.currentTime + (event.seekOffset || defaultSeekTime), audio.duration);
    updatePositionState();
  }
   ...
   */
  playPauseButton.addEventListener('click', () => audio.paused ? playOrResumeCurrentTrack() : pause());
  ...
  document.querySelector('#backwardButton').addEventListener('click', seekBackward);
  document.querySelector('#forwardButton').addEventListener('click', seekForward);
  ...
  audio.addEventListener('play', () => {
    document.querySelector(`.track-${previousTrackIndex}`).classList.remove('table-active', 'playing');
    document.querySelector(`.track-${currentTrackIndex}`).classList.add('table-active', 'playing');
  });
